...
export default function DateTaskPanel({ 
  ...
  const isHistoryDate = isPast(selectedDate)
          ...
          {dateTodos.length === 0 ? (
            <div className="py-12 text-center">
              <div className={cn(
                ...
                isHistoryDate ? "bg-gray-100" : "bg-orange-100"
              )}>
                <Clock className={cn("w-8 h-8", isHistoryDate ? "text-gray-400" : "text-orange-400")} />
              </div>
              <p className="text-gray-500">
                {isHistoryDate ? '这一天没有任务记录' : '这一天还没有任务'}
              </p>
              <p className="text-sm text-gray-400 mt-1">
                {isHistoryDate ? '过去的日期无法添加新任务' : '点击下方按钮添加新任务'}
              </p>
            </div>
          ) : (
        ...
        <div className="p-4 border-t border-gray-100">
          {isHistoryDate ? (
            <div className="text-center py-2">
              <p className="text-sm text-gray-400">过去的日期无法添加新任务</p>
            </div>
          ) : (
